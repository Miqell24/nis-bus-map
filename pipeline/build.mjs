...
// Niš: ONE feed for JGSP Niš-Ekspres — 62 city and suburban lines reaching
// Niška Banja, Malča and the villages of the Nišava valley. Buses only; the
// city has neither tram nor trolleybus.
...
// Exact-match repairs for poles a feed names badly. Niš needs none — 847
// stop names and only one shouted, DIS, which is the shop it is named after.
// The direction tail every pole carries is a different matter: see nisName.
const NAME_FIX = {};
...
// ONE feed, ONE cfg.
//
// Generated from JGSP's own timetables (Nedžad Beus's gtfs-generators on
// GitLab, the source Transitous uses for Serbia) and shipped with shapes.
//
// TWO feed habits had to be answered here.
//
// 1. There is ONE ROUTE ROW PER PATTERN — 195 rows for 62 line numbers — and
//    the row's long name is the pattern's own "A - B". That is a gift, because
//    the trips carry NO headsign at all and direction_id is 0 on every one of
//    them: without a destination the engine would key both directions the same
//    and draw only the longer one. `headsignOf` reads the destination out of
//    the route row instead, which is where this feed actually keeps it.
// 2. Every pole is named with its direction in tow — "22. decembar
//    ( ka centru )", "Ledena Stena (od centra)". That is the direction, not
//    the name; the map aggregates poles by name and prints it on the street,
//    so the tail comes off. "(ulaz)" stays — that one really does tell two
//    poles apart.
const LBL = new Map();
const isRailTrunk = () => false;

const DEST = new Map();   // route_id → destination, filled by mapKey below
const nisKey = (sn, r) => {
  if (!sn) return null;
  const ln = (r.route_long_name || '').trim();
  const i = ln.lastIndexOf(' - ');
  if (i > 0) DEST.set(r.route_id, ln.slice(i + 3).trim());
  return sn;
};
const nisName = (n) => n
  .replace(/\s*\(\s*(ka centru|od centra|ka gradu|od grada)\s*\)\s*$/i, '')
  .replace(/\s+/g, ' ')
  .trim();

const MODES = [{
  mode: 'bus', label: 'buses', graphMode: 'road',
  // the city and the valley villages — 2 × 2 tiles
  ...
  feeds: [
    { tag: 'nis', dir: 'data/gtfs', routeTypes: ['3'], mapKey: nisKey,
      headsignOf: (t) => DEST.get(t.route_id) || '', nameFix: nisName },
  ],
}];
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    // Some feeds keep the destination somewhere other than trip_headsign —
    // Niš puts it in the ROUTE row's "A - B" name and leaves every trip blank.
    const hsOf = feed.headsignOf || ((t) => t.trip_headsign);
    const hsKey = (t) => hsFix((hsOf(t) || '').replace(/\s+/g, ' ').trim()) || '0';
    ...
    for await (const t of iterCsv(join(fdir, 'trips.txt'))) {
      ...
      if (e.trips.length < (feed.allVariants ? 1e9 : tripCap)) e.trips.push({ trip_id: t.trip_id, headsign: hsFix((hsOf(t) || '').trim()) });
    }
